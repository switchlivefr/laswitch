...
.field{
  ...
  transition:border-color .2s,box-shadow .2s;caret-color:#1877f2;
}
.field:focus{border-color:#1877f2;box-shadow:0 0 0 3px rgba(24,119,242,.18)}
...
.btn{
  width:100%;background:#1877f2;color:#fff;border:none;border-radius:var(--r);
...
<div class="wrap">
  ...
  <button id="btn-demande-acces" onclick="openAdminM();swLoadAdminRequests()"
...
<!-- ══════════════════════════════════════════════════════════
     MODALE ADMIN : DEMANDES D'ACCÈS — identique à index.html
     ══════════════════════════════════════════════════════════ -->
<div class="moverlay" id="m-sw-admin" onclick="if(event.target===this)closeAdminM()">
  <div class="mbox" style="display:flex;flex-direction:column;max-height:calc(88vh - 64px)">
    ...
    <div class="mfoot" style="flex-direction:row;gap:8px">
      ...
      <button onclick="swAdminAjoutNomFacebook()" style="flex:1;background:#1877f2;border:none;border-radius:8px;padding:10px;color:#fff;font-family:'Bebas Neue',sans-serif;font-size:15px;letter-spacing:1px;cursor:pointer">AJOUT NOM FACEBOOK</button>
    </div>
  </div>
</div>

<!-- Saisie du prénom avant validation — identique à index.html -->
<div id="m-sw-admin-prenom" style="display:none;position:fixed;inset:0;z-index:1100;background:rgba(0,0,0,.72);align-items:center;justify-content:center;padding:24px">
  <div style="background:var(--card);border:1px solid #1877f2;border-radius:var(--r);width:100%;max-width:340px;padding:28px 24px 24px;display:flex;flex-direction:column;gap:16px">
    ...
    <button
      id="sw-admin-prenom-addbtn"
      onclick="swAdminPrenomAjouter()"
      style="width:100%;background:#1877f2;color:#fff;border:none;border-radius:var(--rs);padding:14px;font-family:'Bebas Neue',sans-serif;font-size:18px;letter-spacing:2px;cursor:pointer"
    >AJOUTER</button>
  </div>
</div>
...
/* ── Bouton "AJOUT NOM FACEBOOK" : ferme la modale, révèle la page ── */
function swAdminAjoutNomFacebook() {
  closeAdminM();
}

/* ── Charger les demandes d'accès (identique index.html swLoadAdminRequests) ── */
  ...
  fetch(ADMIN_APPS_SCRIPT_URL + '?action=getAccessRequests')
    ...
    .then(function(data) {
      ...
      rows.forEach(function(row) {
        ...
        var info = '<div style="flex:1"><div style="font-size:14px;font-weight:700;color:' + (isVal?'var(--muted2)':'var(--white)') + '">' + nom + '</div><div style="font-size:11px;color:var(--muted2);font-family:DM Mono,monospace">' + (row.date||'').replace(/\s*\(.*\)\s*/,'') + '</div></div>';
        d.innerHTML = info;
        if (isVal) {
...
/* ── Valider une demande (identique index.html) ── */
...
/* ── swAccCapitalize : utilisé par l'input prénom (repris de index.html) ── */
function swAccCapitalize(inp) {
  var v = inp.value;
  inp.value = v.charAt(0).toUpperCase() + v.slice(1);
}
