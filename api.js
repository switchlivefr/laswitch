...
async function handleRequest(request, env) {
  ...
  if (url.hostname === 'marco.laswitch.net') {
    if (url.pathname === '/manifest-ajoutnomfb.json') {
      const asset = await env.ASSETS.fetch(new Request('https://dummy/manifest-ajoutnomfb.json'));
      return new Response(asset.body, {
        headers: { 'Content-Type': 'application/manifest+json; charset=utf-8', 'Cache-Control': 'no-cache' }
      });
    }
    if (url.pathname === '/icon-192-ajoutnomfb.png') {
      return env.ASSETS.fetch(new Request('https://dummy/icon-192-ajoutnomfb.png'));
    }
    if (url.pathname === '/icon-512-ajoutnomfb.png') {
      return env.ASSETS.fetch(new Request('https://dummy/icon-512-ajoutnomfb.png'));
    }
